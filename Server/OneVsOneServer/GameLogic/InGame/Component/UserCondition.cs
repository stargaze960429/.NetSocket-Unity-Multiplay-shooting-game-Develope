...
namespace OneVsOneServer.GameLogic.InGame.Component
{
    class UserCondition : IComponent
    {
        ...
        {
            if (this.gameObject.GameTime - dieTime > GameDefines.PlayerRespawnTime && IsAlive == false) {
                this.Respawn();
            }
        }

        private void Die(string damageSenderNickname) {

            this.gameObject.ScoreBoard.ReportKill(damageSenderNickname, this.gameObject.Name);
        ...
        public void Damage(int amount, string senderNickname) {
